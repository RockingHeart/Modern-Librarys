...
- String（自删除缺陷起）
    ## 添加
        ...
        [1.12.0] 添加参数为basic_string的operator==
        [1.15.0] 添加非静态成员函数subscript
    ...
    ## 改动
        ...
        [1.10.2] 移除at函数的noexcept属性
        [1.10.3] 添加noexcept条件
        [1.11.0] 将其string_box的长度存储形式更改为“remaining”
        [1.12.0] 修正已知的功能逻辑错误
        [1.13.0] 去掉分配时的浮点计算以优化性能
        [1.14.0] 兼容大小端
    
    ## 保留
        [1.1.6] 参数为(size_t, Option)的resize暂不纳入更新列表，因为日后可能会删除它
        [1.4.0] 以注释的形式保留lower与upper的可读性较高的实现
