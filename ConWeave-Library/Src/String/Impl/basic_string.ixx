...
private:
		...
		)
	{
		...
		self_cache.pointer[self_cache.count] = char_t();
...
private:
		...
		)
	{
		...
		if (is_large_mode()) {
			...
			cache.specs = size;
			cache.pointer[cache.specs] = char_t();
		}
...
private:
	...
	constexpr size_t string_capacity() const noexcept {
		if (is_cache_mode()) {
			return core_t::buffer_size - core_t::cache.specs;
		}
