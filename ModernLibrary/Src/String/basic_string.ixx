...
public:
	...
	template <class CharWrap>
		requires (
		    requires (CharWrap wrap) {
		        typename CharWrap::char_t;
				wrap.size;
				CharWrap::data;
	        }
		)
	constexpr basic_string(const CharWrap& wrap) noexcept {
		assign_init<CharWrap::size>(wrap.data);
	}
